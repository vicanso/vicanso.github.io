...
| 参数      | 类型             | 是否必需 | 说明                                                                                                         |
| :-------- | :--------------- | :------- | :----------------------------------------------------------------------------------------------------------- |
| `path`    | String           | **否**   | **（必需）** 一个正则表达式，用于匹配请求的 URL **路径**。只有路径匹配成功的请求，其响应体才会被此插件处理。不设置则匹配所有路径 |
| `filters` | Array of Strings | **是**   | **（必需）** 一个包含一条或多条**过滤规则**的列表。规则会按照列表中的顺序依次执行。                          |
| `status_codes` | Array of Numbers | **No**   | **(Optional)** 一个包含一条或多条**状态码**的列表。只有响应体状态码匹配的请求，其响应体才会被此插件处理。不设置则匹配所有状态码 |
