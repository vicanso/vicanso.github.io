...
- `file`: 文件缓存，使用文件作为缓存后端，支持LRU驱逐机制，不配置则是文件缓存。格式为`/var/cache/pingap/articles?levels=1:2&inactive=7d&reading_max=1000&writing_max=500&cache_max=100&cache_file_max_size=1MB`，参数说明如下：
