...
| 参数                  | 默认值 | 说明                                                                                                          |
| --------------------- | ------ | ------------------------------------------------------------------------------------------------------------- |
| log_level             | INFO   | 应用日志的输出级别（如`DEBUG`, `INFO`, `WARN`, `ERROR`），支持动态设置日志输出级别，调整后在30秒内生效，。    |
...
强烈建议配置健康检查，`Pingap`会自动剔除不健康的节点，若不配置会默认使用tcp端口检测的方式。
...
💡 最佳实践：务必为生产环境设置合理的超时时间，以防止因后端服务缓慢或无响应而导致连接堆积，最终拖垮整个代理服务。
...
| upstream |        | 匹配成功后，将请求转发到的`Upstream`名称，需要配置为存在的`Upstream`。                                     |
...
| 参数                         | 默认值 | 说明                                                                                   |
| ---------------------------- | ------ | -------------------------------------------------------------------------------------- |
| enable_reverse_proxy_headers | false  | 是否自动添加 X-Forwarded-* 和 X-Real-IP 等标准反向代理请求头。在入网节点强烈建议开启。 |
...
| max_retries          |        | 当连接upstream失败时，可重试连接的次数， 0表示不重试。             |
| max_retry_window     |        | 当连接upstream失败时，可重试连接的窗口时间，若超过则表示不再重试。 |
...
| locations | []     | （必需） 绑定到此`Server`的`Location`名称列表，匹配时按`Location`的权重匹配，并非按顺序匹配 |
