...
| `sticky_header` | String  | `stickiness` 为 `true` 时**是**    | -       | The name of the header to use for session persistence. The plugin will use the value of this header to determine the traffic route, ensuring the same user always visits the same upstream. If a cookie is configured, it will be used first.                           |
| `matcher`       | String  | 否                                 | -       | The regular expression to match the request. The plugin will check the value of `sticky_cookie` or `sticky_header` according to this regular expression. If it matches, the plugin will use the `upstream` configured in the plugin, otherwise it will not be adjusted. |
